...
TEST(MessageSegmentTest, CreateSynAckTest)
{
    ...
    ASSERT_EQ(message_segment::flag::syn | message_segment::flag::ack, msg->get_message_flags());
}
...
{
    message_segment m1 = message_segment(any_source_port, any_destination_port, any_sequence_number,
        any_type, any_flags, any_message);
...
TEST(MessageSegmentTest, OperatorVectorTest)
{
    ...
    ASSERT_EQ(static_cast<std::vector<uint8_t>>(msg),
        std::vector<uint8_t>(
                  {0xff, 0xff, 0x00, 0xab, 0x00, 0x7b, 0xfd, 0x16, 0x04, 0x74, 0x65, 0x73, 0x74}));
}
