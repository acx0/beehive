...
namespace at_command_response_frame_test
{
    ...
    at_command_response_frame get_valid_at_command_response_frame_empty_value()
    {
        ...
        return at_command_response_frame(
            any_frame_id, at_command::INTERFACE_DATA_RATE, at_command_response_frame::status::ok);
    }
    ...
    at_command_response_frame get_valid_at_command_response_frame()
    {
        ...
        return at_command_response_frame(any_frame_id, at_command::SERIAL_NUMBER_HIGH,
            at_command_response_frame::status::ok, buffer);
    }
    ...
    TEST(ATCommandResponseFrameTest, GetValueTest)
    {
        at_command_response_frame frame = get_valid_at_command_response_frame();
        ASSERT_EQ(
            any_address_high, util::unpack_bytes_to_width<uint32_t>(frame.get_value().cbegin()));
    }

    TEST(ATCommandResponseFrameTest, OperatorVectorTest)
    {
        at_command_response_frame frame = get_valid_at_command_response_frame();
        ASSERT_EQ(std::vector<uint8_t>({0x88, 0x01, 0x53, 0x48, 0x00, 0xab, 0xcd, 0xef, 0x01}),
