...
namespace uart_frame_test
{
    uart_frame get_valid_tx_request_64_uart_frame()
    {
        ...
        return uart_frame(std::make_shared<tx_request_64_frame>(tx_request_64_frame(
    ...
    {
        std::vector<uint8_t> frame = get_valid_tx_request_64_uart_frame();
    ...
    TEST(UARTFrameTest, ComputeChecksumEmptyPayload)
    {
        ...
        ASSERT_EQ(uart_frame::compute_checksum(frame.cbegin(), frame.cend()),
            uart_frame::CHECKSUM_TARGET);
    }
    ...
    {
        uart_frame frame = get_valid_tx_request_64_uart_frame();
        ASSERT_EQ(static_cast<std::vector<uint8_t>>(frame),
            std::vector<uint8_t>({0x7e, 0x00, 0x0f, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0xff, 0xff, 0x00, 0x74, 0x65, 0x73, 0x74, 0x40}));
    }
}
